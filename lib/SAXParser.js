...
function parseDirectiveArgs(argsStr) {
  return argsStr.split(SPLIT_DIRECTIVE_ARGS_EXPR).filter(arg => arg.length);
}
    ...
    switch (name) {
      ...
      default:
        if (isValidCustomElementName(name)) {
          this.emit('customElementStartTag', name, attrs, selfClosing, location);
        }
    }
