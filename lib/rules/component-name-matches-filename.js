/** @file Rule: component-name-matches-filename */
...
function onDomModuleStartTag(filename, onError) {
  return function _onDomModuleStartTag(id, attrs, selfClosing, location) {
    ...
    const message = `Expected '${path.basename(filename)}' to declare ` +
      `component '${expectedComponentName}' but it declared '${componentName}'`;
