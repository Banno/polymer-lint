...
const POLYMER_BUILTINS = ['array-selector', 'custom-style', 'dom-bind', 'dom-if', 'dom-repeat', 'dom-template'].reduce((builtIns, name) => {
  builtIns[name] = true;
  return builtIns;
}, {});
...
module.exports = function noMissingImport(context, parser, onError) {
  ...
  const check = (name, location) => POLYMER_BUILTINS[name] || imports[name] || onError({
