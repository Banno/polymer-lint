...
function processPattern() {
  ...
  let cwd = _ref$cwd === undefined ? process.cwd() : _ref$cwd;
  let ext = _ref.ext;

  let suffix = '/**';

  const extensions = (ext || ['html']).map(_ext => _ext.charAt(0) === '.' ? _ext.substr(1) : _ext);
...
module.exports = function resolvePatterns(options) {
  ...
  const paths = options._.map(processPattern(options)).reduce((_paths, pattern) => {
    g = new glob.Glob(pattern, g);
    return _paths.concat(g.found);
  }, []);

  return [].concat(_toConsumableArray(new Set(paths))); // unique paths
};